...
function StatusBadge({ status }: { status: string }) {
  const config: Record<string, { label: string; color: string }> = {
    ...
    CLOSED: { label: "Closed", color: "bg-[var(--surface-hover)] text-[var(--text-secondary)]" },
  };
      ...
      <div className="divide-y divide-slate-100">
        {data.combined.per_kategori.map((row) => {
          ...
          return (
            ...
            >
              <span className="text-[var(--text-secondary)]">{LABEL[row.kategori]}</span>
  ...
  return (
    <div className="space-y-5">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {recon.map((r: {kategori: string; esb: number; fisik: number; selisih: number}, i: number) => (
              <tr
                key={r.kategori}
                className={`border-t border-[var(--border)] ${i % 2 === 1 ? "bg-[var(--surface-hover)]/50" : ""}`}
              >
      ...
      <div className="bg-[var(--surface-accent)] border border-[var(--border)] rounded-xl p-5 flex items-center justify-between">
        ...
        <p className="text-[var(--primary)] text-2xl font-bold">{fmt(totalOmzetBersih)}</p>
      </div>
      ...
      {draftExpired && (
        <div className="bg-[var(--surface-hover)] border border-[var(--border)] rounded-lg px-4 py-3 flex items-center justify-between">
          ...
          <button
            ...
            className="text-xs text-[var(--text-tertiary)] hover:text-[var(--text-secondary)] font-medium ml-4 shrink-0"
          >
            Tutup
          </button>
        </div>
