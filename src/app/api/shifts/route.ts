...
// ─── GET /api/shifts ──────────────────────────────────────────────────────────
// Hanya CASHIER dan HEAD_CASHIER — Finance tidak boleh akses halaman shifts
export async function GET(req: NextRequest) {
  ...
  // Head Cashier lihat semua (untuk review)
...
export async function POST(req: NextRequest) {
  ...
  if (activeShift) {
    return NextResponse.json(
      { error: 'Kamu masih memiliki shift aktif. Selesaikan shift sebelumnya terlebih dahulu.' },
      { status: 409 },
    )
  }
  ...
  if (shift_period === ShiftPeriod.SHIFT_2) {
    ...
    if (!shift1Today) {
      return NextResponse.json(
        { error: 'Shift 1 hari ini belum dibuka. Shift 2 hanya bisa dibuka setelah Shift 1 ada.' },
        { status: 422 },
      )
    }
    if (shift1Today.status !== ShiftStatus.CLOSED) {
      return NextResponse.json(
        { error: 'Shift 1 hari ini belum ditutup. Shift 2 hanya bisa dibuka setelah Shift 1 selesai.' },
        { status: 422 },
      )
    }
    if (shift1Today.opened_by === session!.user.id) {
      return NextResponse.json(
        { error: 'Kamu tidak bisa membuka Shift 2 karena kamu adalah kasir Shift 1 hari ini.' },
        { status: 403 },
      )
    }
