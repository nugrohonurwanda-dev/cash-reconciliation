...
// Urutan flat ini HARUS identik dengan urutan PaymentCategory enum di Prisma
export const KATEGORI_LIST = KATEGORI_GROUPS.flatMap((g) =>
  g.items.map((item) => item.key),
) as string[];

// ─── Helpers ──────────────────────────────────────────────────────────────────

/** Cari line berdasarkan kategori key — tidak bergantung urutan array */
function findLine(
...
function grandTotal(lines: TransactionLine[]): number {
  return lines.reduce((sum, l) => sum + parseRupiah(l.nilai ?? "0"), 0);
}
