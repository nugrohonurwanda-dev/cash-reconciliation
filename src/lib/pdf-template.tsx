...
// Types
// ─────────────────────────────────────────────────────────────
interface KategoriRecon {
  kategori: string;
  esb: Prisma.Decimal | number;
  fisik: Prisma.Decimal | number;
  selisih: Prisma.Decimal | number;
}
interface ReconciliationResult {
  per_kategori: KategoriRecon[];
  total_esb: Prisma.Decimal | number;
  total_fisik: Prisma.Decimal | number;
  total_selisih: Prisma.Decimal | number;
}
interface DailyAccumulationResult {
  shift_1: ReconciliationResult | null;
  shift_2: ReconciliationResult | null;
...
export interface PDFData {
  shift: {
    ...
    modal_awal: Prisma.Decimal | number | string;
    variance_note?: string | null;
    opener?: { full_name: string | null; username: string } | null;
    special_logs: Array<{
      tipe: string;
      nominal: Prisma.Decimal | number;
      nomor_bill?: string | null;
      alasan?: string | null;
      nama_member?: string | null;
      metode?: string | null;
      kategori_biaya?: string | null;
    ...
    approvals: Array<{
      ...
      catatan?: string | null;
      approver?: { full_name: string | null; role: string } | null;
    }>;
  };
  ...
  totalOmzetBersih: number;
  dailyAccumulation?: DailyAccumulationResult | null;
}
...
const fmt = (v: unknown): string => `Rp ${safeNum(v).toLocaleString("id-ID")}`;
...
// buildPDFDocument
// ─────────────────────────────────────────────────────────────
export function buildPDFDocument(data: PDFData) {
  const {
    shift,
    recon,
    totalVoid,
  ...
  const reconRows = recon.per_kategori.map((r, i) => {
    ...
    return ce(
      View,
      { key: r.kategori, style: i % 2 === 0 ? s.row : s.rowEven },
      ce(
        Text,
        { style: { ...s.td, flex: 2 } },
  ...
  const specialLogRows = shift.special_logs.map((log, i) => {
    ...
    return ce(
      View,
      { key: i, style: i % 2 === 0 ? s.row : s.rowEven },
      ce(
        Text,
        { style: { ...s.td, flex: 1.5 } },
        TIPE_LABEL[log.tipe] ?? log.tipe,
      ),
  ...
  const signBoxes =
    shift.approvals.length > 0
      ? shift.approvals.map((a, i) =>
          ce(
            View,
            { key: i, style: s.signBox },
            ce(
              Text,
              { style: s.signRole },
              ROLE_LABEL[a.approver?.role ?? ""] ?? a.approver?.role ?? "-",
  ...
  const accumRows = dailyAccumulation
    ? dailyAccumulation.combined.per_kategori
        .map((row, i) => {
          const r1 = dailyAccumulation.shift_1?.per_kategori.find(
            (r) => r.kategori === row.kategori,
          );
          const r2 = dailyAccumulation.shift_2?.per_kategori.find(
            (r) => r.kategori === row.kategori,
          );
          ...
          return ce(
            View,
            { key: row.kategori, style: i % 2 === 0 ? s.row : s.rowEven },
            ce(
              Text,
              { style: { ...s.td, flex: 2 } },
            ...
            ce(
              Text,
              { style: { ...s.tdBold, flex: 2, textAlign: "right" } },
              vt > 0 ? fmt(vt) : "\u2014",
            ),
          );
        })
    ...
    ce(
      ...
      ce(
        ...
        ce(
          ...
          ce(
            Text,
            { style: s.headerGenerated },
            `Digenerate: ${fmtDateTime(new Date())}`,
          ),
        ),
      ),
      ce(
        ...
        ce(
          Text,
          { style: s.titleText },
          "Hand Over Kasir \u2014 Rekonsiliasi Kas",
        ),
        ce(
          View,
          {},
      ...
      ce(
        ...
        ce(
          ...
          ce(
            Text,
            { style: { ...s.tdTotalWhiteRight, flex: 2 } },
            fmt(totalEsb),
          ),
          ce(
            Text,
      ...
      ...(dailyAccumulation
        ? [
            ce(
              View,
              { style: s.accumSection },
              ce(
                Text,
                { style: s.accumTitle },
                "Rekap Akumulasi Hari Ini (Shift 1 + Shift 2)",
              ),
              ce(
                View,
                { style: s.table },
                ...
                ce(
                  ...
                  ce(
                    Text,
                    { style: { ...s.tdTotalWhite, flex: 2 } },
                    "Total Fisik",
                  ),
                  ce(
                    Text,
                    { style: { ...s.tdTotalWhiteRight, flex: 2 } },
                    fmt(dailyAccumulation.shift_1?.total_fisik ?? 0),
                  ),
                  ce(
                    Text,
                    { style: { ...s.tdTotalWhiteRight, flex: 2 } },
                    fmt(dailyAccumulation.shift_2?.total_fisik ?? 0),
                  ),
                  ce(
                    Text,
                    { style: { ...s.tdTotalWhiteRight, flex: 2 } },
                    fmt(dailyAccumulation.combined.total_fisik),
                  ),
                ),
              ),
            ),
      ...
      ce(
        ...
        ce(
          View,
          { style: s.logCard },
          ce(Text, { style: s.logLabel }, "Total Void"),
          ce(Text, { style: s.logValRed }, fmt(totalVoid)),
        ),
        ce(
          View,
          { style: s.logCard },
        ...
        ce(
          View,
          { style: s.logCard },
          ce(Text, { style: s.logLabel }, "Total Other Cost"),
          ce(Text, { style: s.logValViolet }, fmt(totalOtherCost)),
        ),
      ),
      ...
      ce(
        ...
        ce(
          ...
          ce(
            Text,
            { style: s.omzetSub },
            "Fisik \u2212 Void \u2212 Discount \u2212 Other Cost",
          ),
        ),
        ce(
          View,
